...
- `Publish-TemplateMirror.ps1`: publishes the contents of a private workspace's local `templates/` mirror to the GitHub repository root. It previews by default; use `-Push` only after reviewing the staged diff and sensitive scan.

Example NAS import data lives in `data/NAS_USERS_IMPORT.example.csv` after publication, or in `templates/data/NAS_USERS_IMPORT.example.csv` inside a private workspace mirror. Real employee lists, NAS addresses, group mappings, tokens, and execution logs must stay in a private workspace.
