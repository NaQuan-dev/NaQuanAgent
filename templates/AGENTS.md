...
This is not a public framework overview. The private workspace root is the runtime area. Files under `templates/` are sanitized scaffolds only and must never be treated as live rules.
...
- Files under `templates/` named `AGENTS.md`, `SUB_AGENT.md`, `group_context.md`, or `agent_rules/*` are never live runtime rules.
