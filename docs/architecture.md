...
`agent_rules/` provides reusable rule templates:
...
`SUBAGENTS.md` indexes reusable sub-agent templates. It is a template repository file, not live runtime routing.

`subagent/` is for projects, groups, departments, or business domains:
...
`subagents/new_media/` is a more complete scenario template for content strategy, topic planning, scripts, asset libraries, and pre-publish review. It contains only placeholders and no real organization, product, customer, or account data.

## Memory Review Layer

`memory_review/` is for weekly memory compression, knowledge draft review, and `COMMON_ERRORS.md` update candidates:
...
`scripts/` contains skeletons only:
...
`data/` provides redacted example data structures, including users, groups, group membership details, user-group views, group-message indexes, and NAS import planning. Real runtime data must be synchronized by trusted private connectors and must not be committed.
